...
//更改学生密码
studentSchema.statics.changepw = function(sid,oldpw,newpw,callback){
    this.find({"sid" : sid},function(err,results){
        ...
        if(results.length == 0){
            ...
            var sha256 = crypto.createHash("sha256");
            if(sha256.update(oldpw).digest("hex")!=thestudent.password){
                callback("密码不正确");
            }else{
                //加密字符串，digest表示进一步处理为hex十六进制
                var sha256 = crypto.createHash("sha256");
                thestudent.password = sha256.update(newpw).digest("hex");
                thestudent.save();
                callback("成功修改！");
            }
        }
...
//重置学生密码
studentSchema.statics.resetpw = function(sid,pw,callback){
    this.find({"sid" : sid},function(err,results){
        if(err){
            callback("服务器错误！请检查输入的内容！");
            return;
